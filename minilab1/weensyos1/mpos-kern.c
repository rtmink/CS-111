...
static pid_t do_newthread(process_t *parent, int start_function);    // *** Extra-credit 6
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		// *** Exercise 3 ***
		// Check if the current process has a process
		// waiting on it
		if (current->p_waiter_pid)
		{
			// a process is waiting on a current process
			process_t *caller = &proc_array[current->p_waiter_pid];

			// Since there is a process waiting on the current
			// process, we set its waiter_pid to 0
			// and indicate that it is no longer a process
			current->p_waiter_pid = 0;
			current->p_state = P_EMPTY;

			// Unblock the blocked process
			caller->p_registers.reg_eax = current->p_exit_status;
			caller->p_state = P_RUNNABLE;
		}
	...
	case INT_SYS_WAIT: {
		...
		else if (proc_array[p].p_state == P_ZOMBIE)
		{
			...
			// *** Exercise 4 ***
			// The bug was that we forgot to indicate
			// that the requested process is no longer 
			// a process
			//
			// Since the process with pid 'p' has exited and
			// the current process has called sys_wait(),
			// the process with pid 'p' is no longer a process
			proc_array[p].p_state = P_EMPTY;
		}
		else
		{
			...
			// *** Exercise 3 ***
			// Indicate the process with pid 'p'
			// that a process is waiting on it
			proc_array[p].p_waiter_pid = current->p_pid;
		}
		schedule();
	}
	...
	case INT_SYS_KILL:
	{
        ...
        else if (proc_array[p].p_state != P_ZOMBIE)
        {
            ...
            if (proc_array[p].p_waiter_pid)
            {
                // a process is waiting on a current process
                process_t *caller = &proc_array[proc_array[p].p_waiter_pid];
                
                caller->p_state = P_RUNNABLE;
                caller->p_registers.reg_eax = proc_array[p].p_exit_status;
...
do_fork(process_t *parent)
{
	...
	process_t *child;
	int i = 1;
	while(proc_array[i].p_state != P_EMPTY)
	{
		if (++i >= NPROCS)
			return -1; // there is no process empty descriptor
	}

	// we find an empty process descriptor
	child = &proc_array[i];
	child->p_registers = parent->p_registers;

	copy_stack(child, parent);

	child->p_state = parent->p_state; // set the child to be runnable
	child->p_registers.reg_eax = 0;	// indicating that it is the child process
	child->p_pid = i; // set the child's pid back to its original pid
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// copy source process's stack to destination process's stack
	memcpy((void *)dest_stack_bottom, (void *)src_stack_bottom, stack_size);

	// set the destination process's stack pointer
	// to point into its stack
